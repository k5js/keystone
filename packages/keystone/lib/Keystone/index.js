...
module.exports = class Keystone {
  constructor({
    ...
    this._customProvider = new CustomProvider({
      ...
      _buildQueryHelper: this._buildQueryHelper,
    });
    this._providers = [
  ...
  createAuthStrategy(options) {
    ...
    this._providers.push(
      new ListAuthProvider({ authStrategy: strategy, list: this.lists[listKey] })
    );
    return strategy;
  }
  ...
  extendGraphQLSchema({ types = [], queries = [], mutations = [] }) {
    this._customProvider.extendGraphQLSchema({ types, queries, mutations });
  }
